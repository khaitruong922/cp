...
void solve() {
  ...
  for (auto x : tc) {
    ...
    if (myLowerBound == expectedLowerBound)
      ...
      cout << "Incorrect lower bound for " << x
           << " . Expected: " << expectedLowerBound
           << ". Found: " << myLowerBound << endl;

    if (myUpperBound == expectedUpperBound)
      cout << "Upper bound of " << x << " is " << myUpperBound << endl;
    else
      cout << "Incorrect upper bound for " << x
           << " . Expected: " << expectedUpperBound
           << ". Found: " << myUpperBound << endl;

    if (find(v.begin(), v.end(), x) != v.end()) {
      if (myIndex != -1)
        cout << "Found " << x << " at index " << myIndex << endl;
      else
        cout << "Incorrect index for " << x
             << " . Expected: " << find(v.begin(), v.end(), x) - v.begin()
             << ". Found: " << myIndex << endl;
    } else {
      if (myIndex == -1)
        cout << "Did not find " << x << endl;
      else
        cout << "Incorrect index for " << x << " . Expected: " << -1
             << ". Found: " << myIndex << endl;
    }
  }
}
