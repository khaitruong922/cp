...
class SegmentTree {
 public:
  ...
  int build(int l, int r, int i) {
    if (l == r) return st[i] = v[l];
    int mid = getMid(l, r);
    return st[i] = min(build(l, mid, i * 2 + 1), build(mid + 1, r, i * 2 + 2));
  }

  int getMin(int l, int r) { return getMin(0, size() - 1, l, r, 0); }

  int getMin(int ss, int se, int qs, int qe, int si) {
    if (qs <= ss && qe >= se) return st[si];
    if (se < qs || ss > qe) return INT_MAX;
    int mid = getMid(ss, se);
    return min(getMin(ss, mid, qs, qe, si * 2 + 1),
               getMin(mid + 1, se, qs, qe, si * 2 + 2));
  }
  void update(int i, int val) { update(0, size() - 1, i, val, 0); }
